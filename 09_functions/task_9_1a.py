    ...
    for intf in intf_vlan_mapping:
        ...
        for command in access_template:
            if command.endswith('access vlan'):
                result.append(' {} {}'.format(command, intf_vlan_mapping[intf]))
            else:
                result.append(' {}'.format(command))
        if psecurity:
            for command_security in psecurity:
                result.append(' {}'.format(command_security))
        else:
            continue
    return result

output = generate_access_config(access_config, access_mode_template, port_security_template )
